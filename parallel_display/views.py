...
class Texts(forms.Form):
    """
    This class is the form that has dropdown select boxes
    it uses modelchoicefield and choicefield, part of django forms
    ...
    book_dd = forms.ModelChoiceField(
    empty_label = "Select Book", label = "Book", 
    queryset = (BookInfo.objects.all().values_list('title', flat=True)))
    chapter_dd = forms.ChoiceField(label = "Chapter", 
    required=False, choices=choices_final)
...
def get_page(import_page):
    ...
    function for this.
    """
    soup = BeautifulSoup(open(import_page))
    return soup
...
def parse_html(filepath):
    ...
    html_page = str(get_page(filepath))
    p_tag_list = re.findall('<p.*?</p>' , html_page , re.DOTALL)
